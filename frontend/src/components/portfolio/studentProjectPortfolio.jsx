...
export default function StudentProjectProfile({ dis, ...props }) {
    ...
    const share = () => {
        const shareUrl = `${window.location.origin}/main/${projid}`;
    ...
    useEffect(() => {
        ...
        fetchData();
        checklike();
    }, [projid, getstudentdetails, fetchData, checklike]);
                    ...
                    {projdata && (
                        <section className='card-surface p-6 md:p-8 space-y-6'>
                            ...
                            <div className='flex flex-wrap gap-4 text-sm text-ink-500'>
                                ...
                                <button
                                    onClick={handlelike}
                                    className={`flex items-center gap-2 px-3 py-1 rounded-full transition-all duration-300 ${like === 1 ? 'bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 shadow-sm' : 'hover:bg-ink-100 dark:hover:bg-ink-800'}`}
                                >
                                    <FontAwesomeIcon icon={like === 1 ? faHeartSolid : faHeartRegular} />
                            ...
                            <div className='space-y-3 pt-4 border-t border-ink-200 dark:border-ink-700'>
                                ...
                                <div className='grid grid-cols-1 gap-2'>
                                    {students.map((student, index) => (
                                        <div
                                            key={index}
                                            className='flex items-center justify-between p-3 rounded-xl border border-ink-200 dark:border-ink-700 hover:border-brand-300 hover:bg-white dark:hover:bg-ink-800 transition-all group'
                                        >
                                            ...
                                            <span className='text-[10px] uppercase tracking-wider text-ink-400 font-bold'>Member</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
